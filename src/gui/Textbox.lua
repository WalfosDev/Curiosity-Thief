...
function Textbox:init(x, y, width, height, text, loading, font)
    ...
    self.text = text
    self.font = font or gFonts['regular-small']
    _, self.textChunks = self.font:getWrap(self.text, self.width - 6)

    self.chunkCounter = 1
    self.endOfText = false
    self.closed = false

    self.textIsLoading = loading
