Instructions: Your project must be at least as complex as (and distinct from!) the games you’ve implemented in this course, and should really be moreso. Submissions of low complexity may be rejected! You must explain your game in detail and why you believe it to meet the complexity and distinctiveness requirement in a README.md file at the root of your project. 

Hello, the game designed for this final project is called “Curiosity Thief” and is run on the LOVE2D version 0.10.2 windows framework. To start the game just download the game folder and LOVE2D v0.10.2, then drag the game’s folder on to the love.exe icon 
...
The functions to modify a house can be split into 2 categories that I made up which are: intelligent functions and hardcode functions. Intelligent functions do all the generation for you whereas hardcode functions must be hand defined.  

Houses are made by an intelligent function taking the definitions and calling hardcode functions to manufacture the house. The intelligent function is ProcedurallyGenerateRooms().  Now how the house is made varies from house_definitions but basically here is the gist. 
